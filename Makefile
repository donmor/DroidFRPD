...
SDK_DIR := $(shell grep '^sdk\.dir=.*' local.properties | sed 's/sdk\.dir=//g')
ifneq ($(HAS_SDK_DIR),'')
...
--init_conf:
	@if [ -s local.properties ]; then \
	  sed -i '' '/^## This file is automatically generated by Make/d' local.properties; \
	  sed -i '' '/frp.disableClient/d' local.properties; \
	  sed -i '' '/frp.disableServer/d' local.properties; \
	  sed -i '' '/frp.disableArm64/d' local.properties; \
	  sed -i '' '/frp.disableAmd64/d' local.properties; \
	  sed -i '' '/frp.usePrebuiltExecutables/d' local.properties; \
	else \
